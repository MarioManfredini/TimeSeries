...
def kriging_interpolate_residuals(
    ...
    grid_resolution=200,
    variogram_model="linear",
    output_file="kriging_residuals.png"
):
    """
    ...
    ax.set_title("Kriging Interpolation of Model Residuals")
...
def generate_combined_confidence_overlay_image(
    ...
    """
    Combined model (LightGBM, Random Forest, ...) + IDW map with confidence mask and wind arrows.
    """
    ...
    # =========================================================
    combined = pred.reshape(num_cells, num_cells)
...
###############################################################################
def generate_model_idw_labels_image(
    df,
    bounds,
    target,
    vmin,
    ...
    combined_grid = pred_grid.reshape(num_cells, num_cells)
    ...
    print(f"✅ Model + IDW image saved to: {output_file}")
