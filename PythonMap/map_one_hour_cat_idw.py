...
from map_one_hour_cat_idw_helper import (
    compute_cat_loocv_residuals,
    save_cat_idw_formula_as_jpg
)
...
###############################################################################
# === Compute residuals using CatBoost (LOOCV) ===
cat_model, residuals_df, scaler = compute_cat_loocv_residuals(df, features, target)
...
###############################################################################
# === Kriging map of residuals ===
bounds = compute_bounds_from_df(df, margin_ratio=0.10)

interpolated_residuals_image = "kriging_of_cat_residuals.png"
...
html_file = f'map_cat_idw_one_hour_{prefecture_name}_{target}_{year}{month}{day}{hour}.html'
...
###############################################################################
# === Formula image ===
formula_image_path = os.path.join(".", "tmp", "formula_cat_idw.jpg")
os.makedirs(os.path.dirname(formula_image_path), exist_ok=True)

save_cat_idw_formula_as_jpg(formula_image_path)

###############################################################################
# === Labels image ===
labels_image = 'cat_labels_image.png'
...
report_file = f'map_cat_idw_{prefecture_name}_{target}_{year}{month:02}{day:02}_{hour:02}00.pdf'
...
save_model_report_pdf(
    ...
    title=f"CatBoost Interpolation and IDW - {prefecture_name} - {year}/{month}/{day} {hour:02d}H"
)
