...
Author: Mario
Description:
Multi-step (24-hour horizon) forecasting of Ox(ppm) using GRU.
"""
...
BATCH_SIZE = 128
LR = 0.0005
HIDDEN_UNITS = [128, 64]  # List of GRU layers units
DROPOUT = 0.1
PATIENCE = 10
...
ax1.set_title(f"GRU Learning Curve – Best val_loss: {min(history.history['val_loss']):.5f}")
ax1.legend()
ax1.grid(True)
figures.append(fig1)

# Forecast vs True (first horizon)
fig2, ax2 = plt.subplots(figsize=(10, 4))
ax2.plot(y_true_inv[-720:, 0], color='gray', label='True')
ax2.plot(y_pred_inv[-720:, 0], color='lightgray', linestyle='dashed', label='GRU')
ax2.set_title(f"GRU Forecast (t+1)\nR²: {r2:.5f}")
ax2.legend()
ax2.grid(True)
figures.append(fig2)
...
save_report_to_pdf(
    ...
    title=f"GRU Multi-step Forecast Report - {station_name}",
    params=params_info,
    features=features,
    errors=errors,
    figures=figures
