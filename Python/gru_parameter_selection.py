...
"""
Created 2025/11/03
GRU single-step forecast for Ox(ppm)
Author: Mario
"""
...
X_scaled = scaler_X.fit_transform(data_model[features])
y_scaled = scaler_y.fit_transform(data_model[target_col])

X_seq, y_seq = create_sequences(pd.DataFrame(X_scaled, columns=features),
                                pd.DataFrame(y_scaled, columns=target_col),
                                LAG)
...
def build_gru_model(input_shape, hidden_units, dropout):
    """Build a GRU model."""
    model = Sequential()
    model.add(GRU(hidden_units[0], return_sequences=len(hidden_units) > 1,
...
input_shape = (LAG, len(features))
model = build_gru_model(input_shape, HIDDEN_UNITS, DROPOUT)
...
###############################################################################
# === Callbacks ===
MODEL_PATH = "gru_best_model.keras"
...
ax2.plot(y_pred_inv[-720:], label='GRU prediction', color='lightgray', linestyle='dashed')
ax2.set_title(f'GRU Regression (sequence input)\nR²: {r2:.5f}')
...
###############################################################################
# === PDF Report ===
report_file = f'gru_parameter_selection_{station_name}_{prefecture_code}_{station_code}_{target_item}.pdf'
...
save_report_to_pdf(
    ...
    title=f"GRU Parameter Selection Report - {station_name}",
    params=params_info,
    errors=errors,
    figures=figures
)
