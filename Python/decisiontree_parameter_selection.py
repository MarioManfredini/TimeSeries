...
Created 2025/11/09
@author: Mario
DecisionTree parameter selection for Ox(ppm)
"""
...
ax1.plot(y_pred[-720:], label='Decision Tree', color='lightgray', linestyle='--')
ax1.set_title(f'Decision Tree Regression\nR²: {r2:.5f}')
...
ax4.set_title('Top 20 Feature Importances (Decision Tree)')
ax4.grid(True, axis='x', linestyle='--', alpha=0.7)
for bar in bars:
    width = bar.get_width()
    ax4.text(width + 0.005, bar.get_y() + bar.get_height() / 2, f"{width:.3f}", va='center')
fig4.tight_layout()
figures.append(fig4)
...
###############################################################################
# === Save Report to PDF ===
report_file = f'decisiontree_parameter_selection_{station_name}_{prefecture_code}_{station_code}_{target_item}.pdf'
...
save_report_to_pdf(
    ...
    title=f"DecisionTree Parameter Selection Report - {station_name}",
    params=params_info,
    features=features,
