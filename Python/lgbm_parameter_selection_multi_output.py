...
Created 2025/11/16
@author: Mario
LGBMRegressor parameter selection for Ox(ppm)
"""
...
ax1.plot(y_pred[-720:], label='LGBM', color='lightgray', linestyle='--')
ax1.set_title(f'LGBM Regression\nR²: {r2:.5f}')
...
ax4.set_title('Top 20 Feature Importances (LGBM)')
...
###############################################################################
# === Save Report to PDF ===
report_file = f'lgbm_parameter_selection_{station_name}_{prefecture_code}_{station_code}_{target_item}.pdf'
...
save_report_to_pdf(
    ...
    title=f"LGBM Parameter Selection Report - {station_name}",
    params=params_info,
    features=features,
