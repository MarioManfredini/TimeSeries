...
"""
Created 2026/02/23
NHITS 24h Rolling Forecast (Fixed Model, stride=1) + Time Exogenous
@author: Mario
"""
...
futr_features = ["hour_sin", "hour_cos", "dayofweek", "is_weekend"]

df = df[["unique_id", "ds", "y"] + futr_features].dropna().copy()
...
for start in range(0, n_test - forecast_horizon + 1, stride):
    ...
    futr_df["hour_sin"] = np.sin(2 * np.pi * futr_df["hour"] / 24)
    futr_df["hour_cos"] = np.cos(2 * np.pi * futr_df["hour"] / 24)
    futr_df["dayofweek"] = futr_df["ds"].dt.dayofweek
    futr_df["is_weekend"] = (futr_df["dayofweek"] >= 5).astype(int)
...
explanation = (
    "The N-HiTS model is a hierarchical multi-resolution neural architecture "
    "for multi-step time series forecasting.\n\n"
    "In this configuration, the model uses calendar-based future-known "
    "exogenous variables (hourly cyclical encoding and weekly indicators) "
    "to improve the representation of daily and weekly seasonality.\n\n"
    "The forecast horizon is predicted simultaneously using a direct "
...
params = {
    ...
    "Input size": 720,
    "Future exogenous": ", ".join(futr_features),
...
report_path = os.path.join(
    "..", "reports",
    f"nhits_24h_time_exog_{station_name}_{prefecture_code}_{station_code}_{target_item}.pdf"
)
